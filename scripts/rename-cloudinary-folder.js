...
async function renameFolder() {
  try {
    ...
    console.log(`Found ${feed.length} images to rename...`)
    ...
    for (let i = 0; i < feed.length; i++) {
      ...
      try {
        ...
        // Create new public_id (remove 'portfolio/' prefix)
        const newPublicId = currentPublicId.replace('portfolio/', '')
    ...
    console.log('✅ Updated feed.json with new URLs')
